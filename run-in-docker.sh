...
if [ -z "$DISPLAY_SIZE" ]; then
  DISPLAY_SIZE="1440x900x16"
fi

ENV=" -e SERVER_HOSTNAME=$SERVER_HOSTNAME -e SERVER_BASEPORT=$SERVER_BASEPORT -e GRUNT_OPTIONS=$GRUNT_OPTIONS -e DISPLAY_SIZE=$DISPLAY_SIZE"
echo ENV=$ENV
